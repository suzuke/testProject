    ...
    def start(self):
        ...
        if os.path.isdir(working_directory):
            ...
            for widget in self.customWidgetList:
                widget_list.append((widget.get_filename(), widget.get_path()))

            #single core
            for file in files:
                classify(file, self.workingDirectoryPath.get(), widget_list)

            #multiprocessing.freeze_support()
            #pool = multiprocessing.Pool()
            #pool.starmap(classify, zip(files, repeat(working_directory), repeat(widget_list)))
...
def moveto(filename, from_folder, to_folder):
...
def classify(file, working_path, widget_list):
        ...
        for widget in widget_list:
            if widget[0] in file:
                ...
                if os.path.isdir(to_folder):
                    ...
                    moveto(file, working_path, to_folder)
                #else:
